...
static void UART_DMATransmitCplt(DMA_HandleTypeDef *hdma)
{
	...
	else if((_driver->huart->Instance->CR3 & USART_CR3_HDSEL) != 0){
		/*
		SET_BIT(_driver->huart->Instance->CR1, USART_CR1_RE);
...
uint8_t uartDriverLoadData(const void *data,unsigned bytes,uartDriver *_driver){
	...
	if(_driver->txBuf != NULL){
		...
		if ((_driver->huart->hdmatx->Instance->CCR & DMA_CCR_EN) == 0){
			...
			/*
			if((_driver->huart->Instance->CR3 & USART_CR3_HDSEL) != 0){
				CLEAR_BIT(_driver->huart->Instance->CR1, USART_CR1_RE);
...
void uartDriverDMASync(uartDriver *_driver){
	...
	HAL_DMA_Abort(_driver->huart->hdmarx);
